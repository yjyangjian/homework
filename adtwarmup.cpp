...
STUDENT_TEST("测试 duplicateNegatives"){
    Queue<int> singleNegative = {-2};
    Queue<int> expect = {-2, -2};
        duplicateNegatives(singleNegative);
    EXPECT_EQUAL(singleNegative, expect);
}
STUDENT_TEST("测试 duplicateNegatives"){
    Queue<int> noNegative = {2};
    Queue<int> expect = {2};
        duplicateNegatives(noNegative);
    EXPECT_EQUAL(noNegative, expect);
}
STUDENT_TEST("测试 duplicateNegatives"){
    Queue<int> midNegative = {1, -2, 3};
    Queue<int> expect = {1, -2, -2, 3};
        duplicateNegatives(midNegative);
    EXPECT_EQUAL(midNegative, expect);
}
STUDENT_TEST("测试 duplicateNegatives"){
    Queue<int> topNegative = {-2, 1};
    Queue<int> expect = {-2, -2, 1};
        duplicateNegatives(topNegative);
    EXPECT_EQUAL(topNegative, expect);
}

STUDENT_TEST("测试 duplicateNegatives"){
    Queue<int> bottomNegative = {1, -2};
    Queue<int> expect = {1, -2, -2};
        duplicateNegatives(bottomNegative);
    EXPECT_EQUAL(bottomNegative, expect);
